# -*- coding: utf-8 -*-
"""
Fantasy Draft Optimizer — Streamlit MVP v2.2 (uploads-only, robust normal CDF)

Focused fixes since v2.1:
- **Fix:** Replace np.erf usage with a robust normal CDF implementation that works
  even when NumPy lacks `np.erf`. Order of preference: scipy.special.erf (if present),
  then math.erf vectorized, then a pure-NumPy Abramowitz-Stegun approximation.
- **Fix:** Use CV slider values correctly (v2.1 mistakenly hardcoded defaults).

Other constraints preserved from v2.1:
- No default file paths or disk writes. All inputs are uploads and processed in-memory.
- No new capabilities added; only robustness & bug fixes.
"""
...
def _normal_cdf_numpy_only(x: np.ndarray) -> np.ndarray:
    """Pure-NumPy approximation to Φ(x) via Abramowitz–Stegun (7.1.26) using erf approximation."""
    # erf(x) ≈ sign(x) * (1 - t * exp(-x^2) * poly(t)), t=1/(1+p|x|)
    # Convert Φ(x) = 0.5 * (1 + erf(x / sqrt(2)))
    z = x / np.sqrt(2.0)
    t = 1.0 / (1.0 + 0.3275911 * np.abs(z))
    a1, a2, a3, a4, a5 = 0.254829592, -0.284496736, 1.421413741, -1.453152027, 1.061405429
    poly = (((((a5 * t) + a4) * t) + a3) * t + a2) * t + a1
    erf_approx = np.sign(z) * (1.0 - poly * t * np.exp(-z * z))
    return 0.5 * (1.0 + erf_approx)


def normal_cdf(x: np.ndarray) -> np.ndarray:
    """Standard normal CDF with robust fallbacks (no reliance on np.erf)."""
    x = np.asarray(x, dtype=float)
    # Try SciPy if available (fast, vectorized)
    try:
        from scipy.special import erf as _scipy_erf  # type: ignore
        return 0.5 * (1.0 + _scipy_erf(x / np.sqrt(2.0)))
    except Exception:
        pass
    # Try Python's math.erf and vectorize it
    try:
        vec_erf = np.vectorize(_math.erf)
        return 0.5 * (1.0 + vec_erf(x / np.sqrt(2.0)))
    except Exception:
        # Fallback to pure-NumPy approximation
        return _normal_cdf_numpy_only(x)
...
def _parse_adp_from_bytes(content: bytes) -> pd.DataFrame:
    ...
    # Position (from column or "(POS)" in the player string)
    pos_col = first_present(df.columns, ["pos", "position"])
...
st.set_page_config(page_title="Fantasy Draft Optimizer (Streamlit MVP v2.2)", layout="wide")
st.title("Fantasy Draft Optimizer — Streamlit MVP v2.2")
...
with st.sidebar:
    ...
    st.caption("Upload any subset. ADP is optional. Nothing is saved to disk.")
...
if var.use_logistic and "LogisticScale" in model_df.columns:
    ...
    curve = [1.0 / (1.0 + _math.exp((p - sel_row["ADP"]) / scale_val)) for p in range(1, league.teams * rounds + 1)]
else:
